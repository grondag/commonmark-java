...
public class LinkReferenceDefinitionParser {
	...
	private int label(CharSequence line, int i) {
		final int afterLabel = LinkScanner.scanLinkLabelContent(line, i);
		if (afterLabel == -1) {
			return -1;
		}
		...
		if (afterLabel >= line.length()) {
			...
			final String normalizedLabel = Escaping.normalizeLabelContent(label.toString());
	...
	private int destination(CharSequence line, int i) {
		i = Parsing.skipSpaceTab(line, i, line.length());
		final int afterDestination = LinkScanner.scanLinkDestination(line, i);
		...
		destination = (line.charAt(i) == '<')
				...
				final int afterSpace = Parsing.skipSpaceTab(line, afterDestination, line.length());
	...
	private int title(CharSequence line, int i) {
		final int afterTitle = LinkScanner.scanLinkTitleContent(line, i, titleDelimiter);
		if (afterTitle == -1) {
			// Invalid title, stop
			return -1;
		}
		...
		final int afterTitleDelimiter = afterTitle + 1;
		final int afterSpace = Parsing.skipSpaceTab(line, afterTitleDelimiter, line.length());
		if (afterSpace != line.length()) {
	...
	private void finishReference() {
		...
		final String d = Escaping.unescapeString(destination);
		final String t = title != null ? Escaping.unescapeString(title.toString()) : null;
